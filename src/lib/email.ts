...
export const sendVerificationEmail = async (email: string, userId: string, token: string, settings?:boolean) => {
  ...
  if(settings){
     verificationLink = `https://bytesize.world/api/settings/verify-email?userId=${userId}&token=${token}&email=${email}`;
  }else{
     verificationLink = `https://bytesize.world/api/auth/verify-email?userId=${userId}&token=${token}&email=${email}`;
  }

  const command = new SendEmailCommand({
    Source: 'noreply@bytesize.world',
    Destination: {
      ToAddresses: [email],
    },
    Message: {
      Subject: {
        Data: "Bytesize Email Verification",
      },
      Body: {
        Html: {
          Data: `<p>Thank you for signing up for Bytesize! Please verify your email by clicking the following link:</p><a href="${verificationLink}">${verificationLink}</a>`,
        },
        Text: {
...
export const sendPasswordResetEmail = async (email: string, userID: string, token: string) => {
  const resetLink = `https://bytesize.world/api/auth/reset-password?token=${token}&userId=${userID}&email=${email}`;

  const input = {
    Source: 'noreply@bytesize.world', // Verified sender email
    Destination: {
      ToAddresses: [email],
    },
    Message: {
      Subject: {
        Data: "Bytesize Password Reset",
      },
      Body: {
        Html: {
